...
public class ClassroomEntityConfiguration : IEntityTypeConfiguration<Classroom>
{
    ...
    {
        builder
            .HasQueryFilter(filter: c => c.IsActive);

        builder
            .HasMany(navigationExpression: c => c.Students)
            .WithOne(navigationExpression: s => s.Classroom)
            .HasForeignKey(foreignKeyExpression: s => s.ClassroomId)
